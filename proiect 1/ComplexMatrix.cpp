...
istream& operator>>(istream& in, ComplexMatrix& cm) { //case: empty
    in >> cm.lines;
    in >> cm.col;
    cm.mat = new Complex*[cm.lines];
    for (int i = 0; i < cm.lines; i++) {
        cm.mat[i] = new Complex[cm.col];
    }
    for(int i = 0; i < cm.lines; i++) {
        for(int j = 0; j < cm.col; j++) {
            in >> cm.mat[i][j];
...
ComplexMatrix ComplexMatrix::operator+(const ComplexMatrix& cm) {
    ...
    ComplexMatrix result(NULL, lines, col);
...
ComplexMatrix ComplexMatrix::module() const {
    ...
    for(int i = 0; i < lines; ++i) {
        ...
        for(int j = 0; j < col; ++j) {
            ...
            result.mat[i][j] = Complex(sqrt(absSquared), 0); // Imaginary part is 0
        }
    }
...
void ComplexMatrix::set_elem(int line, int col, Complex **c) {
    mat[line][col] = **c;
}